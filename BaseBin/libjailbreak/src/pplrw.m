...
uint64_t walkPageTable(uint64_t table, uint64_t virt, bool *err)
{
	...
	if ((table1Entry & 0x3) != 3) {
		JBLogError("[walkPageTable] table1 lookup failure, table1Entry:0x%llX, table1Off: 0x%llX, table:0x%llX virt:0x%llX", table1Entry, table1Off, table, virt);
		if (err) *err = true;
		return 0;
	}
	...
	switch (table2Entry & 0x3) {
		...
		default:
			JBLogError("[walkPageTable] table2 lookup failure, table2Entry:%0llX, table:0x%llX virt:0x%llX", table2Entry, table2, virt);
			return 0;
	}
}
