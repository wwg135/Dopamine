...
- (void)performBackup {
    ...
    "echo \"******插件设置恢复成功******\"\n\n"
    "echo \"******检查文件夹权限******\"\n"
    "libraryPath=\"/var/jb/User/Library\"\n"
    "if [ \"$(stat -c %U:%G \"$libraryPath\")\" != \"mobile:mobile\" ]; then\n"
    "    chown -R 501:501 \"$libraryPath\"\n"
    "fi\n"
    "if [ \"$(stat -c %a \"$libraryPath\")\" != \"755\" ]; then\n"
    "    chmod 755 \"$libraryPath\"\n"
    "fi\n"
    "find \"$libraryPath\" -type d -exec chmod 755 {} +\n"
    "find \"$libraryPath\" -type f -exec chmod 644 {} +\n"
    "echo \"******权限检查修改成功******\"\n\n"
    "echo \"******正在准备注销生效******\"\n"
    "sleep 1s\n"
